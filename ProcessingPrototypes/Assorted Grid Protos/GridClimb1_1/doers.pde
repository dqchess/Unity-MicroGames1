...
void AddTilesInTopRow(int numTiles) {
  for (int i=0; i<numTiles; i++) {
    int count=0;
    while (true) {
      GridSpace randSpace = GetSpace(RandCol(), 0);
...
void AddRandTile() {
  int count=0;
  while (true) {
    GridSpace randSpace = GetSpace(RandCol(), RandRow());
    if (randSpace.getTile() == null) { // This space is vacant!
      addTile(randSpace.col,randSpace.row, RandColor(),0);
      break;
    }
    // Safety check.
    if (count++ > 999) { println("Whoa! No vacant board spaces left!"); return; }
  }
}
...
void determineWhatTilesCanMoveInPreview() {
  ...
  MoveTiles(previewDirX, previewDirY, false);
...
void MoveTiles(int dirX,int dirY) { MoveTiles(dirX,dirY, false); } // HACK in
void MoveTiles(int dirX,int dirY, boolean isPreviewMove) {
