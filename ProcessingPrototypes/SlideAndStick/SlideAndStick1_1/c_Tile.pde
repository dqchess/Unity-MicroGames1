...
class Tile {
  int col,row;
  int colorID;
  int groupID; // FOR NOW, tiles move together simply, by groupID. (Instead of merging into cool shapes.)
  float x,y;
  ...
  Tile clone() {
    Tile cloneTile = new Tile(col,row, colorID);
    cloneTile.x = x;
    cloneTile.y = y;
    cloneTile.targetX = targetX;
    cloneTile.targetY = targetY;
    cloneTile.groupID = groupID;
