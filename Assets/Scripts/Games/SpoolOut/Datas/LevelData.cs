...
namespace SpoolOut {
    public class LevelData {
        ...
        public LevelData (LevelAddress myAddress, BoardDataXML ldxml) {
    		...
    		// LOAD up stats!
    		DidCompleteLevel = SaveStorage.GetInt (SaveKeys.SpoolOut_DidCompleteLevel(myAddress)) == 1;
    	}
    	...
    	public void SetDidCompleteLevel (bool _didCompleteLevel) {
    		if (DidCompleteLevel != _didCompleteLevel) {
    			...
    			SaveStorage.SetInt (SaveKeys.SpoolOut_DidCompleteLevel(myAddress), DidCompleteLevel?1:0);
    		}
    	}
