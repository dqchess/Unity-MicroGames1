...
//Spool GetSpoolPathEnd(int col,int row) {
//  Spool spoolHere = GetSpool(col,row);
//  if (spoolHere == null) { return null; }
//  // Ah, there's a Spool here, but it's not its last space.
//  if (!spoolHere.IsLastSpace(col,row)) { return null; }
//  // There's a Spool here and this is its last space!
//  return spoolHere;
//}
Spool GetTopSpool(int col,int row) {
  if (GetSpace(col,row) == null) return null;
  return GetSpace(col,row).GetTopSpoolOnMe();
}
...
boolean CanSpoolPathEnterSpace(Spool spool, Vector2Int spacePos) {
...
boolean IsSpaceOpen(Vector2Int spacePos) {
  ...
  if (!space.CanAddSpoolPath()) { return false; }
  return true;
}

//Boolean CanSpoolMoveInDir(Spool tile, int dirX,int dirY) {//, int groupID) {
...
//  if (gridSpace.GetSpoolOnMe() != null) { return false; } // Another Spool? Can't move in dir.
//// if (tile!=null&&gridSpace.getSpool()!=null && !canSpoolsMerge(tile, gridSpace.getSpool())) return false;
//  return true;
//}
...
color GetFillColor(int colorID) {
  switch (colorID) {
    ...
    case 5: return color( 80,170,245);
    case 6: return color( 12,170,245);
    default: return color(random(255),150,245); // Hmm.
  }
}
//color GetStrokeColor(int colorID) {
color GetStrokeColor(color fillColor) {
//  color fillColor = GetFillColor(colorID);
  return color(hue(fillColor), saturation(fillColor), brightness(fillColor)*0.7);
//  switch (colorID) {
...
//    case 5: return color( 80,150,160);
//    case 6: return color( 12,150,160);
