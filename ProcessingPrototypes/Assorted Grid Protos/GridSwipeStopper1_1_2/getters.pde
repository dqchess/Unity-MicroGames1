...
GridSpace GetRandSpace(int fromDirX, int fromDirY) {
  int nextCol,nextRow;
  if (fromDirX==-1) nextCol = cols-1;
  else if (fromDirX==1) nextCol = 0;
  else nextCol = RandNextCol();
  if (fromDirY==-1) nextRow = rows-1;
  else if (fromDirY==1) nextRow = 0;
  else nextRow = RandNextRow();
  if (nextCol==-1 || nextRow==-1) { return null; } // Safety check.
  return gridSpaces[nextCol][nextRow];
}
