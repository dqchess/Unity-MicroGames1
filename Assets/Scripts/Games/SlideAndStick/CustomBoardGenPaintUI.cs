...
namespace SlideAndStick {
    public class CustomBoardGenPaintUI : MonoBehaviour {
        ...
        // ----------------------------------------------------------------
        private void Start() {
            // Color buttons to match Tile colors!
            for (int i=0; i<b_colorIDs.Length; i++) {
                ColorBlock cb = b_colorIDs[i].colors;
                Color bodyColor = TileViewBody.GetBodyColor(i);
        ...
        private void PaintSpace(BoardSpace space, int colorID) {
            ...
            // NOT the eraser?? Add a Tile here!
            if (colorID >= 0) {
                boardData.tileDatas.Add(new TileData(space.BoardPos, colorID));
            }
